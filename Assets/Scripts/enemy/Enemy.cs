...
public class Enemy : MonoBehaviour
{
    ...
    private void PerformAttack()
    {
        ...
        if (playerTransform != null)
        {
            CharacterStats characterStats = playerTransform.GetComponent<CharacterStats>();
            if (characterStats != null)
            {
                characterStats.CurrentHealth -= attackPower; // 플레이어의 체력 감소
            }
        }
    }
    ...
    {
        isAttacking = true;
        InvokeRepeating("PerformAttack", 0f, attackSpeed); // 일정 간격으로 공격
    }

    private void StopAttacking()
    {
        isAttacking = false;
        CancelInvoke("PerformAttack"); // 공격 중지
    }
